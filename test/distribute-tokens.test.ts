...
describe("Testing wallet with verified sig", () => {
  ...
  describe("Deploys a wallet contract", async () => {
    ...
    it("Should send some tokens out of the wallet", async () => {
      ...
      await tokenInstance.transfer(walletAddress, toMint);
      expect(await tokenInstance.balanceOf(walletAddress)).to.eq(toMint);

      let values = new Array(100);
      values.fill(toPay);

      let payee = addressesToSend[2];

      // Generate transfer data
      let stabTransferTx = await tokenInstance.populateTransaction.distribute(addressesToSend, values);
      // let stabTransferTx = await tokenInstance.populateTransaction.distribute([payee, payee], [toPay, toPay]);
      // let stabTransferTx = await tokenInstance.populateTransaction.transfer(payee, toPay);
      ...
      let payeeBalanceAfter = await tokenInstance.balanceOf(payee);
